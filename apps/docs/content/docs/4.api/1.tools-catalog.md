...
Reads are in `repo-explorer`; `code-review` also adds `createCommitComment`; `maintainer` includes all five.

| Tool | Capability | Write |
|---|---|---|
| `getCommitComment` | read a single commit comment by ID | — |
| `listCommitComments` | list comments on a commit | — |
| `createCommitComment` | comment on a commit, optionally anchored to a diff line | Yes |
| `updateCommitComment` | edit an existing commit comment | Yes |
| `deleteCommitComment` | delete a commit comment | Yes |
