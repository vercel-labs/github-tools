...
`options` mirrors `EveGithubToolsOptions` below (`token`, `connector`, `connect`, `preset`, `tools`, `requireApproval`, `overrides`, `author`/`committer`/`coAuthors`). Full config table and Connect setup: [eve extension guide](/frameworks/eve-extension#config-schema). Package README: [`packages/github-tools-eve-extension`](https://github.com/vercel-labs/github-tools/tree/main/packages/github-tools-eve-extension).

## `createGithubTools(options)` — eve (deprecated)

::callout{icon="i-lucide-alert-triangle"}
**Deprecated** — use [`githubExtension`](#githubextensionoptions--eve-extension) from `@github-tools/eve-extension` instead. This direct import keeps working but is no longer the recommended path.
::
...
Requires optional peer dependencies `eve` and **`ai` v7** — see [Installation](/getting-started/installation) and [eve (direct import)](/frameworks/eve).
...
`tools` is an exact allow-list of tool names — pass it alone to hand-pick a handful of tools, or alongside `preset` to further narrow it (the effective set is the intersection of both). Also exports individual eve tool factories (`listPullRequests()`, `createIssue()`, …) for one-tool-per-file layouts. Approval supports `once`, predicates, and eve helper passthrough — unlike the Workflow subpath, approval **is enforced** at runtime.
