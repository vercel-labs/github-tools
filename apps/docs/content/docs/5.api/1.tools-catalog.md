...
| `getFileContent` | read a file at a specific path and ref | No |
...
| `getPullRequest` | read PR details including diff stats | No |
| `listPullRequestFiles` | list files changed in a PR with diff status and patches | No |
...
| `getIssue` | read issue details and comments | No |
...
| `getLatestRelease` | get the latest published release (excludes drafts and prereleases) | No |
...
| `getCommit` | read a single commit with its diff | No |
| `getBlame` | line-level git blame for a file (GraphQL) | No |
| `compareCommits` | compare two branches, tags, or commits: ahead/behind counts, commits in between, and files that differ | No |
