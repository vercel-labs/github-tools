...
type GithubToolsOptions = {
  ...
  requireApproval?: ToolApprovalMode | Partial<Record<GithubWriteToolName, ToolApprovalMode>>
...
type ToolApprovalMode = boolean | 'auto'

// Tuning for 'auto' approval and preset: 'auto'. Every field is optional.
type GithubEvaluationOptions = {
  model?: Experimental_EvaluationModel // default 'typesafe-ai/jev-latest'
  maxRisk?: number // default 0.5, on a 0–2 scale
  minIntent?: number // default 0.8
  minPresetProbability?: number // default 0.5
  maxPresets?: number // default 2
}

// Providers receive the tool call the token is requested for (undefined outside a tool call).
type GithubTokenInput = string | ((call?: GithubTokenCall) => Promise<string>)

type GithubTokenCall = {
  toolName: GithubToolName
  input: unknown // after context defaults
  owner?: string // undefined for tools without a repository target
  repo?: string
}
...
`preset: 'auto'` selects presets per call from the latest user message, and `requireApproval: 'auto'` skips approval for low-risk writes the user asked for. See [Let the agent pick presets](/guide/presets#let-the-agent-pick-presets) and [Auto approval](/guide/approval-control#auto-approval).
...
type ConnectGithubToolsOptions = GithubToolsOptions & {
  connect?: GithubConnectParams | GithubConnectParamsResolver
}
...
type GithubConnectParamsResolver = (call?: GithubTokenCall) => GithubConnectParams | Promise<GithubConnectParams>
