...
`connectGithubApproverAuth()` identifies the person approving the write. `githubRepositoryApprover()` then uses the agent GitHub credential to check that person's permission for the input `owner` and `repo`; it uses `GITHUB_TOKEN` unless you pass `agentToken`. The responder must complete the channel's GitHub authentication when prompted. Tools without string `owner` and `repo` inputs are rejected by this policy, so use a per-tool map when only repository tools should require it.
