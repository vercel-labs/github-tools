...
- Prefer the eve extension for new agents (https://github-tools.com/frameworks/eve-extension); use examples/eve as the project layout reference
...
You now have a GitHub agent that can read repos, review PRs, triage issues, and manage CI, with every write operation gated behind durable approval by default. New agents should use the [eve extension](/frameworks/eve-extension) instead; the runnable starter is [`examples/eve/`](https://github.com/vercel-labs/github-tools/tree/main/examples/eve) (or `pnpm dev:eve` from the monorepo root).
