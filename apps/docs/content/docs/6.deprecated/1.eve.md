...
Add `eve` `>=0.34.0` and **`ai` v7** alongside the SDK:
...
Ensure `ai` resolves to **v7** — eve is not compatible with AI SDK v6. You still need `GITHUB_TOKEN` (or pass `token` explicitly). See [Installation](/getting-started/installation).
...
Default (no `requireApproval`): all write tools → `always()`. Unlisted write tools keep the `always()` fail-safe default. Read tools never require approval unless you explicitly set `overrides.<tool>.approval`.
...
```ts [Selected write tools]
import { createGithubTools, githubRepositoryApprover } from '@github-tools/sdk/eve'
import { connectGithubApproverAuth } from '@github-tools/sdk/connect/eve'

const approver = githubRepositoryApprover({
  auth: connectGithubApproverAuth('github'),
...
export default createGithubTools({
  authorizeApprovalResponse: {
    mergePullRequest: approver,
    createPullRequest: approver,
  },
})
```
::

`minimumPermission` accepts `read`, `triage`, `write` (the default), `maintain`, or `admin`. `connectGithubApproverAuth()` identifies the person approving the write. `githubRepositoryApprover()` then uses the agent GitHub credential to check that person's permission for the input `owner` and `repo`; it uses `GITHUB_TOKEN` unless you pass `agentToken`. The responder must complete the channel's GitHub authentication when prompted. Tools without string `owner` and `repo` inputs are rejected by this policy, so use a per-tool map when only repository tools should require it.
...
| `requireApproval` | Global or per-write-tool request approval policy |
| `authorizeApprovalResponse` | Optional global or per-write-tool responder authorization policy |
