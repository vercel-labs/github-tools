...
For [eve](https://eve.dev) — filesystem-first agents with durable human-in-the-loop approval — use `@github-tools/sdk/eve` and export `createGithubTools()` from `agent/tools/github.ts`. Requires **`ai` v7** and the `eve` package.

See [eve agents](/guide/eve-agents).
