...
`requireApproval` is enforced by `WorkflowAgent` via `needsApproval` on write tools — the workflow pauses until the user approves or denies. Wire `WorkflowChatTransport` on the client for resumable streams. More detail: [Durable workflows (Vercel Workflow)](/guide/durable-workflows).

## eve agent (filesystem-first)

Minimal [eve](https://eve.dev) agent using `@github-tools/sdk/eve` with the `code-review` preset. One file under `agent/tools/` registers every GitHub tool via `defineDynamic`, with durable approval that pauses the session until a human approves writes.

The full example lives in [`examples/eve-agent/`](https://github.com/vercel-labs/github-tools/tree/main/examples/eve-agent):
...
Requires **`ai` v7** and `eve` v0.19+. More detail: [eve agents](/guide/eve-agents).

## GitHub bot (Chat SDK + Workflow)

A complete, durable PR review agent connected to GitHub via [Chat SDK](https://chat-sdk.dev). When someone @mentions the agent on a PR, a durable workflow runs the `code-review` agent — reading files, commits, and diffs using GitHub tools — then posts the review as a comment. Follow-up messages resume the same workflow.

The full example lives in [`examples/pr-review-agent/`](https://github.com/vercel-labs/github-tools/tree/main/examples/pr-review-agent). It combines four primitives in ~60 lines of code:
...
| **eve** agent | `createGithubTools` from `@github-tools/sdk/eve` | filesystem-first agents, durable HITL approval (`once`, predicates) |
