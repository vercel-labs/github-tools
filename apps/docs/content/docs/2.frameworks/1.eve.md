...
description: Create a complete GitHub agent in 3 files with eve — defineDynamic registers all 47 tools, with durable human-in-the-loop approval (always, once, predicates).
seo:
  title: Build a GitHub agent with eve
  description: Create a complete GitHub agent in 3 files — all 47 tools from one file, with durable human-in-the-loop approval.
navigation:
  title: eve
  icon: i-custom:eve
path: /frameworks/eve
...
[eve](https://eve.dev) is Vercel's filesystem-first agent framework: an agent is a folder with instructions, a model config, and tools. With `@github-tools/sdk/eve`, that folder becomes a **complete GitHub agent in 3 files** — all 47 tools registered from a single file, with durable human-in-the-loop approval that actually pauses the session until a person approves.
...
Per-session GitHub tokens via eve connections (`auth: 'eve'`) are planned, backed by [Vercel Connect](/guide/tokens-and-auth#mint-tokens-with-vercel-connect) — for now pass `token` explicitly or set `GITHUB_TOKEN`.
