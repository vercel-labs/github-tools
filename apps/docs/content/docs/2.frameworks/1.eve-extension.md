...
The extension registers each tool with an **authored inline** `execute` that only closes over a serializable tool `name`, then dispatches to the SDK. That pattern survives multi-turn eve Workflow replay (see [#51](https://github.com/vercel-labs/github-tools/issues/51)). Prefer this mount over the deprecated [`@github-tools/sdk/connect/eve`](/deprecated/eve) and [`@github-tools/sdk/eve`](/deprecated/eve) direct imports for Slack / multi-turn durable agents — those register tools from inside `node_modules` and are skipped on replay.
