...
description: Mount @github-tools/eve-extension under agent/extensions/ to add all 75 GitHub tools to an eve agent, the recommended way to wire GitHub into eve, with durable approval and Vercel Connect support.
seo:
  title: Build a GitHub agent with the eve extension
...
[eve](https://eve.dev) is Vercel's filesystem-first agent framework: an agent is a folder with instructions, a model config, and tools. `@github-tools/eve-extension` packages all 75 GitHub tools as a mountable [eve extension](https://eve.dev/docs/extensions): a single `pnpm add` and a one-line mount under `agent/extensions/`, no CLI setup, and no direct SDK import in `agent/tools/`.
...
```ts [agent/extensions/github.ts]
export default githubExtension({
  preset: 'maintainer', // all 75 tools
