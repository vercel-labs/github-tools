...
| Preset | Repository access | Contents | Pull requests | Issues | Actions | Checks/Statuses | Administration |
|---|---|---|---|---|---|---|---|
| `repo-explorer` | selected repos | `read` | `read` | `read` | `read` | `read` | none |
| `code-review` | selected repos | `read` | `read` (or `write` for comments/reviewer requests) | none | none | `read` | none |
| `issue-triage` | selected repos | `read` | none | `write` | none | none | none |
| `ci-ops` | selected repos | `read` | none | none | `write` | `read` | none |
| `security-audit` | selected repos | `read` | `read` | `write` (to report findings) | `read` | `read` | none |
| `release-manager` | selected repos | `write` (for release creation) | `read` | none | `write` (if triggering pipelines) | none | none |
| `maintainer` | selected repos | `write` | `write` | `write` | `write` | `read` | `write` (for repo creation and forking) |
...
For agents deployed on Vercel, [Vercel Connect](https://vercel.com/docs/connect) replaces long-lived PATs entirely. You attach a **GitHub connector** (a Vercel-managed GitHub App) to your project, and your server code requests a **short-lived, scoped token** at runtime, with no secret to store, rotate, or leak.
...
**Using eve?** Pass `connector` directly to the [eve extension](/frameworks/eve-extension#vercel-connect) mount config, no separate import needed:
