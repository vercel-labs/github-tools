...
### Authorize eve approval responders

To restrict who can approve GitHub writes, create a second, user-scoped Connect provider and pass it to `githubRepositoryApprover()`. This provider identifies the person who approves; the app credential checks that person's repository permission and executes the write.

```ts [agent/tools/github.ts]
import {
  connectGithubApproverAuth,
  connectGithubToken,
  connectGithubTools,
} from '@github-tools/sdk/connect/eve'
import { githubRepositoryApprover } from '@github-tools/sdk/eve'

const connector = 'github/my-connector'
const preset = 'maintainer'

export default connectGithubTools(connector, {
  preset,
  authorizeApprovalResponse: githubRepositoryApprover({
    auth: connectGithubApproverAuth(connector),
    agentToken: connectGithubToken(connector, { preset }),
    minimumPermission: 'maintain',
  }),
})
```

The user-scoped provider requests `read:user` by default so it can identify the responder. Set `agentToken` to the lazy app credential because the app, rather than the responder, checks repository permission. `connectGithubTools()` uses its own app credential to execute the write. eve prompts the responder to authenticate through the channel before it settles the approval. `githubRepositoryApprover()` rejects a tool when its input does not contain string `owner` and `repo` fields, so configure it per tool when your agent includes writes without repository inputs.
...
- [`connectGithubApproverAuth` (eve)](/api/reference#connectgithubapproverauthconnector-params--eve) — user-scoped responder identity provider
