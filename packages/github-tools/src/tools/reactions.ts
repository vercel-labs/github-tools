...
export const listIssueReactions = (token: GithubTokenInput): GithubTool =>
  tool({
    ...
    execute: async args => listIssueReactionsStep({ token: await resolveGithubToken(token, githubTokenCall('listIssueReactions', args)), ...args }),
  })
...
export const addIssueReaction = (token: GithubTokenInput, { needsApproval = true }: ToolOptions = {}): GithubTool =>
  tool({
    ...
    execute: async args => addIssueReactionStep({ token: await resolveGithubToken(token, githubTokenCall('addIssueReaction', args)), ...args }),
  })
...
export const listCommentReactions = (token: GithubTokenInput): GithubTool =>
  tool({
    ...
    execute: async args => listCommentReactionsStep({ token: await resolveGithubToken(token, githubTokenCall('listCommentReactions', args)), ...args }),
  })
...
export const addCommentReaction = (token: GithubTokenInput, { needsApproval = true }: ToolOptions = {}): GithubTool =>
  tool({
    ...
    execute: async args => addCommentReactionStep({ token: await resolveGithubToken(token, githubTokenCall('addCommentReaction', args)), ...args }),
  })
