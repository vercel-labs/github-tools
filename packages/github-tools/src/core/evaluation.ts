 ...
 * rates it low-risk and asked for by the latest user message.
 */
export async function needsAutoApproval(
  toolName: GithubWriteToolName,
  input: unknown,
  messages: ModelMessage[],
  { model = DEFAULT_EVALUATION_MODEL, maxRisk = 0.5, minIntent = 0.8 }: GithubEvaluationOptions = {},
): Promise<boolean> {
...
export async function selectPresets(
  messages: ModelMessage[],
  { model = DEFAULT_EVALUATION_MODEL, minPresetProbability = 0.5, maxPresets = 2 }: GithubEvaluationOptions = {},
  ...
  const selected = ranked.filter(preset => answers[preset].probability >= minPresetProbability).slice(0, maxPresets)
